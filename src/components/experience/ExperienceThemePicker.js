...
export default function ExperienceThemePicker({
  className = "",
  theme,
  onChange,
  overrides = {},
  ...
  return (
    <div className={`${styles.picker} ${className}`} aria-label="Color scheme">
      {EXPERIENCE_THEMES.map((option) => {
        const presentation = { ...option, ...overrides[option.id] };

        return (
          <button
            type="button"
            className={theme === option.id ? styles.active : ""}
            style={{ "--theme-swatch": presentation.color }}
            onClick={() => onChange(option.id)}
            aria-label={`${presentation.label} color scheme`}
